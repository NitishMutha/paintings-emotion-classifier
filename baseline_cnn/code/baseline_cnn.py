...
np_filename_yprobs = '../model/baseline_yprobs_wh_bn_nom_drop'
...
with tf.device(device):
    ...
    layer_3 = DenselyConnectedLayer(layer_2.output,16,64,1024,'relu',keep_prob,False,True) # /4
...
with tf.Session() as sess:
    sess.run(init)
    y_true_labels = tf.argmax(dataset_test.labels, dimension=1)
    y_pred_labels = tf.argmax(y, dimension=1)
    ...
    if(TRAINING_MODE):
        ...
        if(CONFUSION_MATRIX):
            ...
            feed_dict={x: dataset_test.images, y_: dataset_test.labels, keep_prob: 1.0,
                                           phase_train: False}
            y_p = sess.run([y_pred_labels],feed_dict)
            # Plot confusion matrix on test data
            save_confusion_matrix(sess, cf_filename, dataset_test, emotion_classes, y_pred_cls)
        else:
            pass
